...
# Overridable so the rendering can be exercised against a plain file off-device.
FB_DEVICE = os.environ.get("Z3PAD_FB", "/dev/fb0")
RES = (640, 480)          # w, h
BPP = 4
FULL = RES[0] * RES[1]
...
def reset_screen():
    ...
    Skipped when FB_DEVICE is a plain file, which has no ioctls to speak of.
    """
    if not stat.S_ISCHR(os.fstat(_fb.fileno()).st_mode):
        return
